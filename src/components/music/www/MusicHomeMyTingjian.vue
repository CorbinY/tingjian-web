...
<script>
  ...
  export default {
    ...
    ,mounted() {
      //发布nav导航信息
      // LocalStorage.set('tagName', this.constant.musicHomeMyTingjian.navName);
      // VueEvent.$emit('tagName',LocalStorage.get('tagName'));
      // this.tagName=this.constant.musicHomeMyTingjian.navName;
      // VueEvent.$emit('tagName',this.tagName);
      Axios.post(this.constant.musicHomeMyTingjian.api,this.constant.page).then(response=>{
